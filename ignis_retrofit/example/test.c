...
void runTests(Tests *tests) {
    for (unsigned int i = 0; i < tests->size; i++) {
        ...
        {
            char *run_command = (char *)malloc((strlen("../../target/debug/san_kyuu_leek ") + strlen(test.binary_path)) * sizeof(char));
            sprintf(run_command, "../../target/debug/san_kyuu_leek %s", test.binary_path);
            // printf("RUNNING: %s\n", run_command);
